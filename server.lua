...
--The following part Register all the cards as Item by looping through the lua table [ cards ]
local totalcard = total + total2
print(totalcard)

for i, card in ipairs(cards) do

	--Contain the name of the card as a string
	local registeredItem = card

	ESX.RegisterUsableItem(registeredItem, function(source) 
		local card = registeredItem
		local xPlayer = ESX.GetPlayerFromId(source)
		TriggerClientEvent('pcards:Show', source, registeredItem)
		print("clientTriggerd!")
	end)
	
end

--This do the same thing for rarecards
